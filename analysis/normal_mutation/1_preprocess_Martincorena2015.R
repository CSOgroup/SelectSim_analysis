...
# normal_mutations/1_preprocess_Martincorena2015.R
...
# R script to preprocess skin mutation data from Martincorena2015.R
# https://pmc.ncbi.nlm.nih.gov/articles/PMC4471149 
...
##### MARTINCORENA_2015 SKIN TISSUE #####

# Load mutations from normal tissue
input_maf <- readxl::read_xlsx("data/raw/normal_mutations/Martincorena2015/NIHMS63718-supplement-2.xlsx", sheet=1, skip=23, col_names = T)
input_maf <- input_maf[!is.na(input_maf$gene_name), ]
colnames(input_maf)[1] <- "sample"
input_maf$subject <- sapply(input_maf$sample, function(x) return(substr(x,1,7)) ) 
colnames(input_maf)[15] <- "gene"
colnames(input_maf)[16] <- "summary"
...
mutation_type = list(
  'ignore' = c("Synonymous"),
  'truncating'= c('Inframe Deletion','Frameshift Deletion','Nonsense','Essential_Splice'),
  'missense' = c('Missense')
)

saveRDS(input_maf, file="data/preprocessed/normal_mutations/Martincorena2015_maf.rds")
...
GAM_full_Martincorena <- pmax(GAM_m_bin,GAM_t_bin)
save(GAM_full_Martincorena, file=paste0("results/normal_mutations/GAM_Martincorena2015_",mutset,".RData"))
...
save(run_data_selectSim, file=paste0("results/normal_mutations/run_data_Martincorena2015_forSelectSim_",mutset,".RData"))
