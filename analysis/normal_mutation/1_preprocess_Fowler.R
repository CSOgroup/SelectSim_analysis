...
##### FOWLER_2021 SKIN TISSUE #####

# Load mutations from normal tissue
normal_samples <- c("PD43996", "PD37619", "PD43991", "PD43943", "PD43994", "PD43995", "PD43992", "PD37612", "PD30272", "PD30274", "PD37184", "PD37182", "PD30273", "PD37275") # from pdf supplementary T1
SuppT4 <- readxl::read_xlsx("data/raw/normal_mutations/Fowler2021/21598290cd201092-sup-248092_3_supp_6645104_q35h43.xlsx", sheet=2, skip=4, col_names = T)
input_maf <- SuppT4[SuppT4$subject %in% normal_samples, ]
...
mutation_type = list(
  'ignore' = c("silent"),
  'truncating'= c('ess_splice','frameshift','inframe','nonsense','splice_region','start_lost','stop_lost'),
  'missense' = c('missense')
)

saveRDS(input_maf, file="data/preprocessed/normal_mutations/Fowler2021_maf.rds")
...
maf_m$mut_filter <- paste0( maf_m$gene, ":", sapply(maf_m$protein_desc, function(x) substr(x,3,nchar(x)-1) ) )
...
save(run_data_selectSim, file=paste0("results/normal_mutations/run_data_Fowler2021_forSelectSim_",mutset,".RData"))
