...
# R script to run selectSim on skin datasets
rm(list=ls())
...
# Run using package SelectSim
  ...
  # load(paste0("results/normal_mutations/run_data_SKIN_forSelectSim_",mutset,".RData"))
  # 
  ...
  #                         maxFDR = 0.25)
  # 
  # saveRDS(result_obj, file=paste0("results/normal_mutations/results_SKIN_",mutset,".rds"))


  load(paste0("results/normal_mutations/run_data_SKIN_forSelectSim_",mutset,".RData"))

  result_obj <- readRDS(paste0("results/normal_mutations/results_SKIN_",mutset,".rds"))
  ...
  blca_samples <- split(names(samples.class), samples.class)[["SKCM"]]
  
  load(paste0("results/normal_mutations/GAM_SKIN_",mutset,".RData"))
  ...
  # signif_genes_sub <- intersect(signif_genes_sub, rownames(GAM_full_SKIN))
  signif_genes_sub <- intersect(signif_genes, rownames(GAM_full_SKIN))
  ...
  gfreq_bladder <- Matrix::rowSums(GAM_full_SKIN[signif_genes_sub,]) / ncol(GAM)
  gfreq_bladder_per_pat <- lapply(blocks, function(x) Matrix::rowSums(GAM_full_SKIN[signif_genes_sub,x])/ length(x) )
  ...
  df_freq <- data.frame( gene=c(signif_genes_sub,signif_genes_sub,signif_genes_sub),
                         mutation_freq = c(gfreq_BLCA, gfreq_pancan, gfreq_bladder),
                         cohort = c(rep("Skin cancer (TCGA-SKCM)", length(gfreq_BLCA)), rep("All cancers (TCGA)", length(gfreq_pancan)), rep(paste0("Normal skin (Martincorena 2015 + Fowler 2021)"), length(gfreq_bladder)))
  )
  # mm <- sum(GAM_mut["FGFR3",blca_samples]) / length(blca_samples)
  # df_freq <- rbind(df_freq, data.frame(gene="FGFR3", mutation_freq = mm, cohort = "Skin cancer (TCGA-SKCM)"))
  plot_mut_freq <- ggplot( subset(df_freq, cohort != "All cancers (TCGA)"),
    ...
    # scale_fill_manual(values=c("Bladder cancer (TCGA-SKCM)"=params$cols[7],"All cancers (TCGA)"=params$cols4[12], paste0(runtypelabs[runtype]," bladder (Lawson 2020)")=params$cols[9])) +
  ...
  results_blca <- subset(results_all_merged, Cohort=="TCGA" & Tumor_run=="MEL")
  ...
  save_ggplot_custom(paste0("figures/normal_mutations/SKIN_summary_",mutset), plot_final, width=56, height=24)
  save_ggplot_custom_pdf(paste0("figures/normal_mutations/SKIN_summary_",mutset), plot_final, width=56, height=24)
