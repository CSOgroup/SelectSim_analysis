...
# normal_mutations/2_run_ESOPHAGUS.R
...
# R script to run selectSim on esophagus datasets
rm(list=ls())
...
# Run using package SelectSim
  ...
  # load(paste0("results/normal_mutations/run_data_ESOPHAGUS_forSelectSim_",mutset,".RData"))
  # 
  ...
  #                         maxFDR = 0.25)
  # 
  # saveRDS(result_obj, file=paste0("results/normal_mutations/results_ESOPHAGUS_",mutset,".rds"))


  load(paste0("results/normal_mutations/run_data_ESOPHAGUS_forSelectSim_",mutset,".RData"))

  result_obj <- readRDS(paste0("results/normal_mutations/results_ESOPHAGUS_",mutset,".rds"))
  ...
  plot_frac <- ggplot( df_fracs,
    ...
    scale_fill_manual(values=setNames(c("grey", RColorBrewer::brewer.pal(12, "Set3"),"skyblue","lightyellow","darkgreen","goldenrod3","lightpink")[1:(ncol(tt)+1)], c("A",colnames(tt)) ) ) +
  ...
  plot_ht <- ComplexHeatmap::Heatmap(ht, 
                    ...
                    top_annotation = ComplexHeatmap::HeatmapAnnotation(
                      ...
                      col = list(donor = setNames(c(RColorBrewer::brewer.pal(12, "Set3"),"skyblue","lightyellow","darkgreen","goldenrod3","lightpink")[1:ncol(tt)], colnames(tt) ))
                    ),
  ...
  blca_samples <- unlist(split(names(samples.class), samples.class)[c("ESCA_CIN","ESCA_ESCC")])
  
  load(paste0("results/normal_mutations/GAM_ESOPHAGUS_",mutset,".RData"))
  # signif_genes_sub <- union(intersect(signif_genes, rownames(GAM_mut)), c("TP53","FGFR3","PIK3CA"))
  # signif_genes_sub <- intersect(signif_genes, rownames(GAM_mut))
  # signif_genes_sub <- intersect(signif_genes_sub, rownames(GAM_full_ESOPHAGUS))
  signif_genes_sub <- intersect(signif_genes, rownames(GAM_full_ESOPHAGUS))
  ...
  gfreq_bladder <- Matrix::rowSums(GAM_full_ESOPHAGUS[signif_genes_sub,]) / ncol(GAM)
  gfreq_bladder_per_pat <- lapply(blocks, function(x) Matrix::rowSums(GAM_full_ESOPHAGUS[signif_genes_sub,x])/ length(x) )
  ...
  df_freq <- data.frame( gene=c(signif_genes_sub,signif_genes_sub,signif_genes_sub),
                         mutation_freq = c(gfreq_BLCA, gfreq_pancan, gfreq_bladder),
                         cohort = c(rep("ESOPHAGUS cancer (TCGA-ESCA)", length(gfreq_BLCA)), rep("All cancers (TCGA)", length(gfreq_pancan)), rep(paste0("Normal ESOPHAGUS (Martincorena 2018 + Abby 2023)"), length(gfreq_bladder)))
  )
  # mm <- sum(GAM_mut["FGFR3",blca_samples]) / length(blca_samples)
  # df_freq <- rbind(df_freq, data.frame(gene="FGFR3", mutation_freq = mm, cohort = "ESOPHAGUS cancer (TCGA-ESCA)"))
  plot_mut_freq <- ggplot( subset(df_freq, cohort != "All cancers (TCGA)"),
    ...
    # scale_fill_manual(values=c("Bladder cancer (TCGA-ESCA)"=params$cols[7],"All cancers (TCGA)"=params$cols4[12], paste0(runtypelabs[runtype]," bladder (Lawson 2020)")=params$cols[9])) +
  ...
  plot_mut_freq_per_pat <- ggplot( gfreq_bladder_per_pat,
    ...
    geom_bar(stat="identity", position=position_dodge2(width=0.25,padding=0.2)) + 
    scale_fill_manual(values=circlize::colorRamp2(c(0,1), c("grey", "darkred"))(seq(0,1,length.out=17))) +
  ...
  results_blca <- subset(results_all_merged, Cohort=="TCGA" & Tumor_run=="ESCA")
  ...
  plot_ovr_n2c <- ggplot( subset(results_pack, significance_with_type != "none"),
    ...
    xlab("wES (ESCA)") + ylab("wES (ESOPHAGUS)") + 
  ...
  save_ggplot_custom(paste0("figures/normal_mutations/ESOPHAGUS_summary_",mutset), plot_final, width=56, height=24)
  save_ggplot_custom_pdf(paste0("figures/normal_mutations/ESOPHAGUS_summary_",mutset), plot_final, width=56, height=24)
